...
На днях я подготовила README для одного проекта, который, надеюсь, 
будет
интересен и поучителен для других разработчиков. Так вот, когда я его писала, я
поняла, что несколько лет назад испугалась бы до смерти, если бы наткнулась на
...
* `awk` или `grep` для поиска файлов в проекте по строке или шаблону
...
могу посоветовать [Cygwin][21]. Возможно, я не права, но принимать участие в
